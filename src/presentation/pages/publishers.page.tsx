...
import { PublishersViewModel } from '../viewModels/publishers.viewModel';
...
export function Publishers({ publisherName }: Props) {
  const { t } = useTranslation();
  const itemsRef = useRef<HTMLDivElement>(null);
  const viewModel = useMemo(() => {
    return new PublishersViewModel(container.get(ItemStateService));
  }, []);
  ...
  useEffect(() => {
    viewModel.setPublisher(publisherName);

    if (publisherName && window.innerWidth < 1024 && itemsRef.current) {
  ...
  const publisherData = viewModel.publisherData.value;
  const filteredItemsCount = viewModel.filteredItems.value.length;
  const activePublisherName = viewModel.activePublisherName.value;
        ...
        <div class="lg:col-span-1" style="filter: drop-shadow(4px 4px 0px rgba(255, 0, 255, 0.5));">
          ...
          >
            <div class="flex flex-wrap items-baseline gap-x-4 gap-y-2 content-start">
              {publisherData.publishers.map(publisher => {
                const count = publisherData.counts[publisher];
                const isActive = publisher.toLowerCase() === activePublisherName;
                const fontSizeClass = viewModel.getFontSizeClass(count);

                return (
                  <Link
                    key={publisher}
                    href={`/publishers/${publisher.toLowerCase()}`}
                    class={`
        ...
        <div class="lg:col-span-3 space-y-6" ref={itemsRef}>
          <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-white">
              {activePublisherName ? activePublisherName : t('lists.common.all_items')}
            </h2>
          ...
          {filteredItemsCount === 0 && !viewModel.isLoading.value && (
            <div class="text-center py-20 text-white/40 border border-white/10 rounded-lg">
              <p>{t('lists.publishers.no_items')}</p>
            </div>
          )}
