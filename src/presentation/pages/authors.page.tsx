...
import { AuthorsViewModel } from '../viewModels/authors.viewModel';
...
export function Authors({ authorName }: Props) {
  const { t } = useTranslation();
  const itemsRef = useRef<HTMLDivElement>(null);
  const viewModel = useMemo(() => {
    return new AuthorsViewModel(container.get(ItemStateService));
  }, []);
  ...
  useEffect(() => {
    ...
    if (authorName && window.innerWidth < 1024 && itemsRef.current) {
  ...
  const authorData = viewModel.authorData.value;
  const filteredItemsCount = viewModel.filteredItems.value.length;
  const activeAuthorName = viewModel.activeAuthorName.value;
          ...
          >
            <div class="flex flex-wrap items-baseline gap-x-4 gap-y-2 content-start">
              {authorData.authors.map(author => {
                const count = authorData.counts[author];
                const isActive = author.toLowerCase() === activeAuthorName;
        ...
        <div class="lg:col-span-3 space-y-6" ref={itemsRef}>
          <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-white">
              {activeAuthorName ? activeAuthorName : t('lists.common.all_items')}
            </h2>
