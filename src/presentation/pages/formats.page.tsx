...
import { FormatsViewModel } from '../viewModels/formats.viewModel';
...
export function Formats({ formatName }: Props) {
  const { t } = useTranslation();
  const itemsRef = useRef<HTMLDivElement>(null);
  const viewModel = useMemo(() => {
    return new FormatsViewModel(container.get(ItemStateService));
  }, []);
  ...
  useEffect(() => {
    viewModel.setFormat(formatName);

    if (formatName && window.innerWidth < 1024 && itemsRef.current) {
  ...
  const formatData = viewModel.formatData.value;
  const filteredItemsCount = viewModel.filteredItems.value.length;
  const activeFormatName = viewModel.activeFormatName.value;
        ...
        <div class="lg:col-span-3 space-y-6" ref={itemsRef}>
          <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-white">
              {activeFormatName ? `#${activeFormatName.toUpperCase()}` : t('lists.common.all_items')}
            </h2>
