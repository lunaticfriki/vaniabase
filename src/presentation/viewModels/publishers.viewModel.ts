...
import { PublisherStatistics } from '../../domain/model/publisherStatistics.model';
import { PaginationViewModel } from './pagination.viewModel';

export class PublishersViewModel {
  private itemStateService: ItemStateService;

  activePublisherName = signal<string>('');
  ...
  constructor(itemStateService: ItemStateService) {
    ...
    effect(() => {
      void this.activePublisherName.value;
  ...
  publisherData = computed(() => {
    return PublisherStatistics.fromItems(this.items.value);
  });
  ...
  filteredItems = computed(() => {
    ...
    const active = this.activePublisherName.value.toLowerCase();
    if (!active) return items;

    return items.filter(item => item.publisher.value.toLowerCase() === active);
  });
  ...
  setPublisher(publisherName: string | undefined) {
    this.activePublisherName.value = (publisherName || '').toLowerCase();
  }

  getFontSizeClass(count: number): string {
    const { minCount, maxCount } = this.publisherData.value;
