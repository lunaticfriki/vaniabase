...
void main() {
  ...
  group('DeleteItemCommandHandler', () {
    test('deletes an item owned by the requester', () async {
      final repository = MockItemRepository();
      final item = ItemMother.book();
      when(() => repository.findById(item.id)).thenAnswer((_) async => item);
      when(() => repository.delete(item.id)).thenAnswer((_) async {});

      final handler = DeleteItemCommandHandler(repository);
      ...
      verify(() => repository.delete(item.id)).called(1);
    });
