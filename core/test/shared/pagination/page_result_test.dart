...
void main() {
  group('PageResult', () {
    test('computes totalPages by ceiling totalItems over pageSize', () {
      final result = PageResult<int>(items: const [1, 2], page: 1, pageSize: 10, totalItems: 25);
    ...
    test('hasNextPage is true while page is below totalPages', () {
      final result = PageResult<int>(items: const [], page: 1, pageSize: 10, totalItems: 25);

      expect(result.hasNextPage, isTrue);
    });

    test('hasNextPage is false on the last page', () {
      final result = PageResult<int>(items: const [], page: 3, pageSize: 10, totalItems: 25);
    ...
    test('hasPreviousPage is true past the first page', () {
      final result = PageResult<int>(items: const [], page: 2, pageSize: 10, totalItems: 25);

      expect(result.hasPreviousPage, isTrue);
    });

    test('map transforms items while keeping pagination metadata', () {
      final result = PageResult<int>(items: const [1, 2, 3], page: 2, pageSize: 3, totalItems: 9);
